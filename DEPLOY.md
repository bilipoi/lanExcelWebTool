...
看板视图提供类似 Trello 的卡片式管理，适合任务和项目进度管理。
...
1. **选择状态列**：
   - 在看板视图顶部，从第一个下拉框选择状态列
   - 状态列应该是包含状态值的列（如：待办、进行中、已完成）
   - 系统会自动根据状态值创建不同的看板列

2. **选择展示列**（可选）：
   - 从第二个下拉框选择要在卡片上显示的列（按住 Ctrl 可多选）
   - 第一个选择的列会作为卡片主标题显示
   - 其他选择的列会作为副信息显示在卡片下方
   - 如果不选择，默认显示第一列的内容

**使用看板：**
- **拖拽卡片**：将卡片从一个列拖到另一个列，自动更新状态
- **双击卡片**：跳转到表格视图，并定位到对应行进行编辑
- **查看统计**：每个列标题显示该状态的卡片数量
- **自定义显示**：通过展示列选择控制卡片显示的内容
